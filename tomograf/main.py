...
def save_to_file(image_to_save):
    ...
    if submit_button:
        ...
        save_as_dicom(filename, image_to_save, data_to_save)
...
def read_from_dicom(path):
    ...
    st.write("Data pozyskania zdjęcia: ", datetime.datetime.strptime(ds.AcquisitionDate, "%Y%m%d").strftime("%Y-%m-%d"))
    st.image(ds.pixel_array)


if __name__ == "__main__":
    st.title("Symulator tomografu komputerowego")
    number_of_detectors = st.slider("Liczba detektorów", 60, 220, 120, 10)
    angle = st.slider("Krok alfa emitera", 1, 10, 1, 1)
    span = st.slider("Rozpiętość układu", 10, 200, 120, 10)
    ...
    if st.session_state.clicked and uploaded_file is not None:
        ...
        save_to_dicom_button = st.button("Save to dicom", on_click=save_to_file(end_image_rescaled))
        if iterations:
            ...
            st.image(sin_selected, caption="Sinogram", width=500)
            st.image(img_selected, caption="Output image")
        else:
            st.image(uploaded_file, caption="Basic image")

            sinus_out_rescaled = np.uint8(sinus_out_rescaled)
            st.image(sinus_out_rescaled, caption="Sinogram", width=500)
